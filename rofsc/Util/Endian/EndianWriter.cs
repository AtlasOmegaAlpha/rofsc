...
namespace System.IO
{
    public sealed class EndianWriter : EndianStream
    {
        ...
        public void WriteStreamedBytesFromFile(string path, int chunkSize = 8192)
        {
            using (var inputStream = File.OpenRead(path))
            {
                ...
                {
                    CreateBuffer(bytesRead);
                    Array.Copy(buffer, 0, this.buffer, 0, bytesRead);
                    WriteBuffer(bytesRead, 1);
                }
            }
        }
        #endregion
